...
namespace theRightDirection.WPF.Xaml
{
    ...
    {
        // an attached behavior won't work due to view model validation not picking up the right
        // control to adorn
        ...
        {
            // we'll need to hook up to one of the element's events in order to allow the GC to
            // collect the control, we'll wrap the event handler inside an object living in an
            // attached property don't be tempted to use the Unloaded event as that will be fired
            // even when the control is still alive and well (e.g. switching tabs in a tab control)
        ...
        private class PasswordBindingMarshaller
        {
            ...
            public void UpdatePasswordBox(SecureString newPassword)
            {
                ...
                try
                {
                    // setting up the SecuredPassword won't trigger a visual update so we'll have to
                    // use the Password property
                    _passwordBox.Password = newPassword.ToUnsecureString();
